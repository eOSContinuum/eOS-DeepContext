...
- Required scalar keys:
  ...
  - `description` MUST carry an Anthropic-spec-compliant description: a brief third-person statement of what the skill does, followed by a WHEN line listing trigger conditions and a WHEN NOT line listing non-invocation cases. The Anthropic Agent Skills runtime caps the description at **1024 characters**; descriptions longer than that are silently rejected at slash-command discovery time. Source files that exceed the cap must be trimmed.
