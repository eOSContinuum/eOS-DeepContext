...
- Skills are source-of-truth at `nodes/Skills/<Folder>/<Folder>.md`. Two runtime-discovery layouts are build-emitted output: `.agents/skills/<runtime_name>/SKILL.md` for the Anthropic Agent Skills SDK, and `.claude/skills/<runtime_name>/SKILL.md` for the Claude Code CLI's slash-command autocomplete.
- `.scripts/build.py` MUST create `<runtime-target>/<runtime_name>/` as a directory containing symlinks pointing back to the compound-node contents for every target in `emit_skills.py`'s `SKILLS_TARGET_DIRS`: `SKILL.md` → `<Folder>.md`, and directory symlinks for `scripts/` and `references/` when present.
